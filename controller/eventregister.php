  ...
  if( !isset($_POST['functionname']) ) 
    ...
    if( !isset($aResult['error'])) 
    {
        ...
        if( !is_array($_POST['arguments']) || (count($_POST['arguments']) < 5) ) 
               ...
               $con=new DbConnector();
               $sql = "SELECT * FROM `eventregdetails` WHERE evnEmail=? AND eventuniid=?";
               ...
               if($result = $stmt->get_result()){
                while($row = $result->fetch_assoc()) {
                  ...
                  if($dbemail ==""){
                    $sqlinsert="INSERT INTO `eventregdetails`(`eventuniid`, `evnTeamname`, `evnCapname`, `evnMob`, `evnEmail`) VALUES (?,?,?,?,?)";
